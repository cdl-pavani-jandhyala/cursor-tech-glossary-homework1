...
export function filterGlossaryEntries(query: string): GlossaryEntry[] {
    ...
    return glossaryEntries.filter((entry: GlossaryEntry): boolean => {
        ...
        return entry.tags.some((tag: string): boolean => 
...
function renderGlossary(filterQuery: string = ''): void {
    const container: HTMLElement | null = document.getElementById('glossary-container');
    ...
    container.innerHTML = filteredEntries.map((entry: GlossaryEntry): string => `
        <article class="glossary-entry bg-white rounded-lg shadow-sm p-5">
            ...
            <div class="glossary-tags-container">
                ...
                <div class="flex flex-wrap gap-2" role="list" aria-label="Tags">
                    ${entry.tags.map((tag: string): string => `
